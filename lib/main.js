      ...
      async.map( results, function(res,daCallback) {
        ...
        async.map( newArr, getResource, function(err,result) {
          if (err) {
            ...
            logger.debug('Succeeded with 2nd fetch and with result: %s', util.inspect(result) );
...
function updateConfig( type, verb, body, cb ) {
  ...
  if ( (type == "apps") && ((_.has( body,'developerId' ))) ) {
    var url = opts.orgUrl + '/developers/' + body.developerId + '/apps';
  }
      ...
      async.series( [
        ...
        function (cb) {
          if ( _.contains(opts.list,"apis") ) {
            ...
            cb( null, 'no api bundles to manage' );
          }
        }],
        function(err,res) {
          if (err) {
            logger.fatal('Failed performing import with error: %s', util.inspect(err) );
          }
          else {
            logger.debug('Finished creating bundles');
            logger.debug('Result set after bundling: %s', util.inspect(res) );
            if ( _.contains(res[1], 'doneWithJSON') ) {
              dcb( null, res[1] );
            }
          }
        });
