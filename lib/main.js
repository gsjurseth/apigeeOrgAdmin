...
function postProxy( proxy, cb ) {
  ...
  request({
    ...
    if (err) {
      ...
      cb( null, 'Done adding proxies' );
    }
    else {
      console.log( "Received unexpected response while importing proxy zip: %s", res.statusCode );
      console.log( "Received this error body: %s", util.inspect(res.body) );
    }
  });
}
  ...
  async.waterfall( [
      ...
      async.series( [
        ...
        function(err,res) {
          if (err) {
            ...
            if (opts.debug) {
              console.log( 'Here it is: %s', util.inspect(res) );
            }
            dcb( null, res[1] );
          }
        });
    ...
    function(res,cb) {
      async.map( res, postProxy, function(err,res) {
        if (err) {
          console.log( "Failed importing proxies" );
        }
        else {
          cb(null,'done');
        }
