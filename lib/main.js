...
// Update the config
function updateConfig( type, body, cb ) {
  //var url = path.join( opts.orgUrl,type );
  var url = opts.orgUrl + '/' + type;

  console.log('da url %s', url );
  /*
  opts.reqOpts = getRequestOptions( opts );
  */
  ...
  request({
    ...
    if (err) {
      ...
      console.log( "Received another response while trying to update config: %s", util.inspect(res) );
    }
  });
}
...
function Import( daOpts ) {
  ...
  async.waterfall( [
    ...
    function(res,cb) {
      readJSONFiles( listWithoutAPIS, function(err,res) {
        cb(null,res);
      });
    },
    function(res,cb) {
      _.each( res, function(types) {
        _.each( types, function(type) {
          console.log ('about to handle data: %s', util.inspect(type) );
          updateConfig( type.type.split('.json')[0], type.result, cb );
        });
      });
    }
  ],
